...
dataset = NIDSToNIoTDataset(path, name='Train_Test_Network', train_size=TRAIN_DATA_SIZE)
data = dataset[0]
neg_data = dataset[1]
TRAIN_DATA_SIZE = 100000 
BATCH_SIZE= 2000 
...
NEIGHBOR_SIZE= 50 
LEARNING_RATE = 0.001 
print(f"""Train data size: {TRAIN_DATA_SIZE}, Batch size: {BATCH_SIZE}, 
    Evaluation strategy: {EVALUATION_STRATEGY}, Threshold: {THRESHOLD_EVAL}, 
    Neighbor size: {NEIGHBOR_SIZE}, Learning rate: {LEARNING_RATE}""")
...
def evaluation():
    ...
    df_ton_iot = pd.read_csv('data/TON_IoT_submission/nids/train_test_network/raw/Train_Test_Network.csv')
