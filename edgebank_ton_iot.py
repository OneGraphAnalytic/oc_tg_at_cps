...
from datasets.nids_ton_iot import NIDSToNIoTDataset
...
# Data
##############################################################################################################
    path = osp.join(osp.dirname(osp.realpath(__file__)), './', 'data/TON_IoT_submission', 'nids')
    dataset = NIDSToNIoTDataset(path, name='Train_Test_Network')
    data = dataset[0]
...
##############################################################################################################
    ...
    df.to_csv(f'results/evaluation{"test"}.csv', index=False)
    
    csv_path = f"data/TON_IoT_submission/nids/train_test_network/raw/Train_Test_Network.csv"
    df_ton_iot = pd.read_csv(csv_path)
    df_ton_iot = df_ton_iot[df_ton_iot.index >= TRAIN_DATA_SIZE ]
    df_type = df_ton_iot[["type", "label"]].reset_index(drop=True)
    df_merged = pd.merge(df_type, df, left_index=True, right_index=True, how="inner")
    df_merged.to_csv(f'results/evaluation{"tmp_ton_fps_investigation"}.csv', index=False)

    grouped_stats = df_merged.groupby('type').agg({
        'tps': 'sum',
        'fps': 'sum',
        'tns': 'sum',
        'fns': 'sum'
    ...
    f1 = f1_score(y_trues, y_preds.round())
    f1_reverse = f1_score(1 - y_trues, 1 - y_preds.round())

    print(f'F1 Score: {f1}')
    print(f'F1 Score Reverse: {f1_reverse}')

    f1 = f1_score(y_trues, y_preds.round(), average='weighted')
    print(f'F1 Score Weighted: {f1}')

    f1 = f1_score(1-y_trues, 1-y_preds.round(), average='weighted')
    print(f'F1 Score Weighted: {f1}')
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Edgebank for NIDS Example TON-IoT Dataset")
    ...
    parser.add_argument("--memory_mode", type=str, default='fixed_time_window', help="Memory Mode") # fixed_time_window , unlimited
